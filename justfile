...
_mccw date difficulty:
    time uv run mccw.py generated-weighted-instances/{{date}}-{{difficulty}}

_generate-mcc date difficulty='easy':
    uv run main.py data/{{date}}.json -d "{{difficulty}}" > generated-mcc-instances/{{date}}-{{difficulty}}

count-weighted date difficulty='easy': (_generate-weighted date difficulty) (_mccw date difficulty)
